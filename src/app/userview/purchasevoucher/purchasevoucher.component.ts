...
export class PurchasevoucherComponent implements OnInit,ComponentCanDeactivate {
  isDirty = false;
  supplyattract = [
  ...
  //module
  module = GlobalConstants.purchasevoucher;
  ...
  ngOnInit(): void {
    ...
    localStorage.setItem('vtype', 'purchasevouchers')
    ...
    this.voucherservice.getCompaniesByUser(this.loginuid)
      .subscribe({
        next: (data) => {
          ...
          this.voucherservice.getAll(6, this.loginuid)
      .subscribe({
        next: (vdata) => {
  ...
  retrieveVouchertypes(): void {
    this.voucherservice.getVoucherTypes(6)
      .subscribe({
        next: (data) => {
