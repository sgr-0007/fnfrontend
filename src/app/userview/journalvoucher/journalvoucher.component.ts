...
export class JournalvoucherComponent implements OnInit ,ComponentCanDeactivate {
  isDirty = false;
  supplyattract = [
  ...
  //module
  module = GlobalConstants.journalvoucher;
  ...
  cfilterid: number;
  stfilterid: any;
  subledgs: import("d:/fnaclient/fnaclientnew/src/app/models/subledger").SubLedger[];

  constructor(private uservice: UsermasterService,private subledgservice : SubLedgerService, private voucherservice: VoucherService, private yearlybudgetservice: YearlybudgetserviceService) { }
  canDeactivate(): boolean {
    return !this.isDirty;
  }
  ngOnInit(): void {
    this.voucherInfoArray = [];
    localStorage.setItem('vtype', 'journalvouchers')
    ...
    this.voucherservice.getCompaniesByUser(this.loginuid)
      .subscribe({
        next: (data) => {
          ...
          this.voucherservice.getAll(4, this.loginuid)
          .subscribe({
            next: (vdata) => {
  ...
  retrieveVouchertypes(): void {
    this.voucherservice.getVoucherTypes(4)
      .subscribe({
        next: (data) => {
