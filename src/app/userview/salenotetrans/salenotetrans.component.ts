...
import { Salenoteheaderdetails } from 'src/app/models/salenoteheaderdetails';
...
import { SalenoteService } from 'src/app/services/salenote.service';
import { UsermasterService } from 'src/app/services/usermaster.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-salenotetrans',
  templateUrl: './salenotetrans.component.html',
  styleUrls: ['./salenotetrans.component.css']
})
export class SalenotetransComponent implements OnInit {
  ...
  toDate:Date;
  currentDate:Date ;
  constructor(private _invoiceService:SalenoteService,
  ...
  Vlaues1:selectVlaues[]=[new selectVlaues(1,'New','fiber_new','mat-color-green'),
    ...
    aucinvdetails:Salenoteheaderdetails[];
    tempaucinvdetails:Salenoteheaderdetails[];
    selectedData:Salenoteheaderdetails[]=[];
    ...
    onRowEditCancel(product: Invoicedetails, index: number) {
      ...
      renderAllAUCInvoice(){
        this._invoiceService.getAUCInvoiceDetailsByDate(this.toDate,this.fromDate,this.filteredbranchid).subscribe(
          (response)=>{
                console.log("salenote"+response)
                
                
                 this.aucinvdetails=response;
                 console.log("salenote details header"+this.aucinvdetails)
