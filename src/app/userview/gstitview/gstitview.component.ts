...
@Component({
  selector: 'app-gstitview',
  templateUrl: './gstitview.component.html',
  styleUrls: ['./gstitview.component.css'],
  styles: [`
...
export class GstitviewComponent implements OnInit, ComponentCanDeactivate {
  isDirty = false;
  supplyattract = [
  ...
  //module
  module = GlobalConstants.paymentvoucher;
  ...
  constructor(private subledgservice : SubLedgerService,private uservice: UsermasterService, private voucherservice: VoucherService, private yearlybudgetservice: YearlybudgetserviceService) { }
  ...
  ngOnInit(): void {
    ...
    localStorage.setItem('vtype', 'gstitview')
    ...
    this.voucherservice.getCompaniesByUser(this.loginuid)
      .subscribe({
        next: (data) => {
          ...
          this.voucherservice.getLastVd(data[0].companyid)
            .subscribe({
              next: (data) => {
          ...
          this.voucherservice.getOfficeAccounts(this.loginuid)
            .subscribe({
              next: (data) => {
                ...
                this.AccountOnChange(data[0].accounttypeid)
...
console.log(+ell.debitledger+"-"+ +this.gsttdscgstdataid)
          if (this.newGstDetails != null) {
            if (+ell.debitledger === +this.gsttdscgstdataid || +ell.creditledger === +this.gsttdscgstdataid) {
              this.tdsamount = (+ell.debitamount) + (+ell.debitamount);

              console.log(+this.tdsamount)
            }
            if (+ell.debitledger === +this.gsttdsigstdataid || +ell.creditledger === +this.gsttdsigstdataid) {
...
this.retrieveSubLedgs(_ledgerid);
  ...
  saveDebitVoucher() {
    if(this.voucherForDialog.narrationdr!=null)
    {
  ...
  loadLedgers(actype: number) {
    ...
    this.voucherservice.getDrLedgers()
      .subscribe({
        next: (data) => {
          ...
          this.debitledgers = data.filter(x => x.companytype ==ctype  && x.accounttypeid ==actype );          console.log(this.debitledgers);
        },
        error: (e) => console.error(e)
      });
    ...
    this.voucherservice.getCrLedgers()
      .subscribe({
        next: (data) => {
          let ctype = this.companies.find(x=>x.companyid == this.voucherForDialog.companyid).companytypeid;
          this.ledgers = data.filter(x => x.companytype ==ctype  && x.accounttypeid ==actype );          console.log(this.ledgers);
        },
        error: (e) => console.error(e)
      });
  }
  ...
  BranchOnChange(company: number) {
    ...
    this.voucherservice.getLastVd(company)
      .subscribe({
        next: (data) => {
          this.vsequence = (data != null) ? +data[0].voucherseq + 1 : 1;
          this.vstartsequence = this.vsequence;
...
if(this.cfilterid == undefined && this.radioSelect != undefined)
{
  this.vouchers = data.filter(x=> x.accounttypeid== +this.radioSelect);

}
if(this.radioSelect == undefined && this.cfilterid != undefined )
{
