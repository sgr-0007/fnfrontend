...
@Component({
  selector: 'app-salesvoucher',
  templateUrl: './salesvoucher.component.html',
  styleUrls: ['./salesvoucher.component.css'],
  styles: [`
...
export class SalesvoucherComponent implements OnInit, ComponentCanDeactivate {
  isDirty = false;
  supplyattract = [
  ...
  ngOnInit(): void {
    ...
    localStorage.setItem('vtype', 'salesvouchers')
    ...
    this.voucherservice.getCompaniesByUser(this.loginuid)
      .subscribe({
        next: (data) => {
          ...
          this.voucherservice.getAll(5, this.loginuid)
          .subscribe({
            next: (vdata) => { 
  ...
  retrieveVouchertypes(): void {
    this.voucherservice.getVoucherTypes(5)
      .subscribe({
        next: (data) => {
  ...
  resetVoucherDetails() {
    this.voucherForDialog = {
      companyname: null!, vouchertypename: null!, narration: null!,
      debitledger: null!, debitamount: null!, creditledger: null!, creditamount: null!, narrationdr: null!,
      narrationcr: null!, companyid: null!, branchid: null!
    };
  ...
  LedgerOnChange(_ledgerid: number) {
    ...
    // }
    console.log("damount="+this.dacheck)
    console.log("dledger="+this.voucherForDialog.debitledger)
  ...
  saveGst() {
    console.log("igst" + this.gstDetails.igstvalue)
    console.log("cgst" + this.gstDetails.cgstvalue)
    console.log("sgst" + this.gstDetails.sgstvalue)
  ...
  InvOnChange(invid: number) {
    this.invobj = new Gstdetails();
    this.invobj = this.invoice.find(x => x.invoiceid == invid);
    console.log(this.invobj);
    this.gstDetails.invoicedate = this.invobj.invoicedate;
    this.gstDetails.igstvalue = this.invobj.igstvalue;
    this.gstDetails.sgstvalue = this.invobj.sgstvalue;
    this.gstDetails.cgstvalue = this.invobj.cgstvalue;
    this.gstDetails.invoicevalue = this.invobj.invoicevalue;
    this.gstDetails.invoicenumber = this.invobj.invoicenumber;
